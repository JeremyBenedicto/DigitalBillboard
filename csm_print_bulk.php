...
$stmt = $conn->prepare("SELECT * FROM client_satisfaction_responses WHERE id IN ($placeholders) ORDER BY id DESC");
if (!$stmt) {
    http_response_code(500);
...
while ($result && ($row = $result->fetch_assoc())) {
    ...
    $responses[] = [
        ...
        'officeCustomName' => $officeCustomName,
...
if (count($responses) === 0) {
    ...
    echo 'No matching responses found.';
    exit;
}
...
<head>
  ...
  <style>
    ...
    .response-start { page-break-before: always; break-before: page; }
...
<body>
  ...
  <?php foreach ($responses as $idx => $r): ?>
    <div class="container page1 <?= $idx > 0 ? 'response-start' : '' ?>">
      <header>
        <div style="width:100%">
          ...
          <p><b>- oOo -</b></p>
          <h2><?= e($r['officeTitle'] !== '' ? $r['officeTitle'] : ('PAMBAYANG TANGGAPAN NG ' . ($r['officeCustomName'] !== '' ? $r['officeCustomName'] : $r['officeSelected']))) ?></h2>
        </div>
      </header>
      ...
      <div class="form-section">
        <div class="form-group inline-group">
          ...
          <label><span class="check-box"><?= mark(!empty($r['clientType']['mamamayan'])) ?></span> Mamamayan</label>
          <label><span class="check-box"><?= mark(!empty($r['clientType']['negosyo'])) ?></span> Negosyo</label>
          <label><span class="check-box"><?= mark(!empty($r['clientType']['gobyerno'])) ?></span> Gobyerno</label>
        </div>

        <div class="form-group inline-group">
          <label>Petsa:</label> <span class="display-input"><?= e($r['visitDate']) ?></span>
          <label>Kasarian:</label>
          <label><span class="check-box"><?= mark(!empty($r['gender']['lalaki'])) ?></span> Lalaki</label>
          <label><span class="check-box"><?= mark(!empty($r['gender']['babae'])) ?></span> Babae</label>
          <label>Edad:</label> <span class="display-input" style="min-width:80px"><?= e($r['age']) ?></span>
        </div>

        <div class="form-group">
          <label>Rehiyon:</label>
          <span class="display-input" style="display:block; margin-top: 5px; min-height: 40px;"><?= e($r['region']) ?></span>
        </div>

        <div class="form-group">
          <label>Uri ng transaksyon o serbisyo:</label>
          <span class="display-input" style="display:block; margin-top: 5px; min-height: 40px;"><?= e($r['transactionType']) ?></span>
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="checkbox-group">
          <label><span class="check-box"><?= mark(!empty($r['cc1'][0])) ?></span> 1. Alam ko ang CC at nakita ko ito sa napuntahang opisina</label>
          <label><span class="check-box"><?= mark(!empty($r['cc1'][1])) ?></span> 2. Alam ko ang CC pero hindi ko ito nakita sa napuntahang opisina</label>
          <label><span class="check-box"><?= mark(!empty($r['cc1'][2])) ?></span> 3. Nalaman ko ang CC nang makita ko ito sa napuntahang opisina</label>
          <label><span class="check-box"><?= mark(!empty($r['cc1'][3])) ?></span> 4. Hindi ko alam kung ano ang CC at wala akong nakita sa opisina</label>
        </div>

        <label>CC2. Kung alam ang CC, masasabi mo ba na ang CC ng opisina ay...</label>
        <div class="checkbox-group">
          <label><span class="check-box"><?= mark(!empty($r['cc2'][0])) ?></span> 1. Madaling makita</label>
          <label><span class="check-box"><?= mark(!empty($r['cc2'][1])) ?></span> 2. Medyo madaling makita</label>
          <label><span class="check-box"><?= mark(!empty($r['cc2'][2])) ?></span> 3. Mahirap makita</label>
          <label><span class="check-box"><?= mark(!empty($r['cc2'][3])) ?></span> 4. Hindi makita</label>
          <label><span class="check-box"><?= mark(!empty($r['cc2'][4])) ?></span> 5. N/A</label>
        </div>

        <label>CC3. Kung alam ang CC, gaano nakatulong ang CC sa transaksyon mo?</label>
        <div class="checkbox-group">
          <label><span class="check-box"><?= mark(!empty($r['cc3'][0])) ?></span> 1. Sobrang nakatulong</label>
          <label><span class="check-box"><?= mark(!empty($r['cc3'][1])) ?></span> 2. Nakatulong naman</label>
          <label><span class="check-box"><?= mark(!empty($r['cc3'][2])) ?></span> 3. Hindi nakatulong</label>
          <label><span class="check-box"><?= mark(!empty($r['cc3'][3])) ?></span> 4. N/ A</label>
        </div>
      </div>
    </div>

    <div class="container page-break page2">
      ...
      <table>
        ...
        <tbody>
          <?php foreach ($questions as $qIdx => $question): ?>
            <tr>
              <td class="question">SQD<?= (int)$qIdx ?>. <?= e($question) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 1) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 2) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 3) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 4) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 5) ?></td>
              <td class="mark-cell"><?= sqdMark($r['sqd'], $qIdx, 6) ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
      ...
      <div class="form-section">
        ...
        <span class="display-input" style="display:block; min-height: 80px;"><?= e($r['suggestions']) ?></span>
        <label style="margin-top:8px; display:block;">Email address (opsyonal):</label>
        <span class="display-input" style="display:block;"><?= e($r['email']) ?></span>
      </div>
